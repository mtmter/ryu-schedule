...
予定とタスクを、月・週カレンダーやタスク一覧で確認できるWebアプリです。

ReactからFastAPIの予定・タスクを取得し、SQLiteに保存された内容を表示します。
予定・タスクの追加フォームは、カレンダーから開く画面を今後実装するため一時的に画面から外しています。
...
> [!IMPORTANT]
> `events`と`tasks`のAPIは実装済みです。
> 追加フォームは、カレンダーから開く画面を今後実装するため一時的に画面から外しています。
...
1. 月表示に予定と未完了タスクが表示されることを確認する
2. 「週」を押し、予定が時刻に応じた位置へ表示されることを確認する
3. 「タスク」を押し、未完了タスクと折りたたまれた完了済みタスクを確認する
4. タスクのチェックを切り替え、未完了と完了済みの間を移動することを確認する
